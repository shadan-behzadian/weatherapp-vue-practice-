<template>
  ...
  <header id="app-header" v-if="info != null">
    <icon
      v-bind:weatherStatus="weatherIcons(info.current.weather[0].main)"
      class="current-icon"
    ></icon>
    <div class="current-info">
      ...
      <p class="current-wind">
        Wind: {{ windDirection(info.current.wind_deg) }}
        {{ convertWindSpeed(info.current.wind_speed) }}kmh
      </p>
    </div>
  </header>
</template>
...
export default {
  ...
  //main data recieved as props from App.vue
  props: {
  ...
  methods: {
    ...
    //converts wind degree from number to letter N, NE, S ...
